...
import { t, getCurrentLang, setupLanguageSwitcher, translateText, isPlaceholderDesc } from "./i18n.js";
...
const renderProject = (project) => {
  ...
  if (project.description) {
    let summaryText = project.description;
    if (isPlaceholderDesc(summaryText)) {
      summaryText = t("noSummaryAvailable", {}, lang);
    }
    const summaryP = element("p", "mod-banner__summary", summaryText);
    heading.append(summaryP);

    if (lang === "zh" && !showOriginalContent && !isPlaceholderDesc(project.description)) {
  ...
  const transBadge = element(
    "span",
    `content-trans-badge ${isTranslatedActive ? "" : "is-original"}`,
    isTranslatedActive ? t("transBadgeMT", {}, lang) : t("transBadgeOrig", {}, lang)
  );

  const transHint = element(
    "span",
    "content-trans-hint",
    isTranslatedActive ? t("transHintMT", {}, lang) : t("transHintOrig", {}, lang)
  );
  ...
  if (isPlaceholderDesc(rawContent)) {
